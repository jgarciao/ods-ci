*** Settings ***
Documentation  Component to interact with Prometheus using API calls
Library        Collections
Library        Process
...
*** Keywords ***
Run Query
  [Documentation]  Run a Prometheus query using the API
...
Get Rules
  [Documentation]  Get Prometheus rules
...
Verify Rule
  [Documentation]  Verify that a Prometheus rule exist, failing if it doesn't
...
Verify Rules
  [Documentation]  Verify that a list of Prometheus rules exist, failing if one or more doesn't
